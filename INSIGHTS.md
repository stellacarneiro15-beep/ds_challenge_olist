...
The benchmark tested weighted logistic regression, XGBoost, LightGBM, and HistGradientBoosting. XGBoost is the preferred production candidate because it slightly outperformed HistGradientBoosting across validation F1, PR-AUC, thresholded F1, and Brier score in the current benchmark table. HistGradientBoosting remains a simpler pure scikit-learn fallback, but dependency simplicity is not the deciding constraint here.

On the held-out test period, the selected XGBoost row achieved validation F1 0.266, PR-AUC 0.136, Brier score 0.103, and F1 0.044 at a validation-selected threshold of 0.682 in the displayed benchmark. The no-skill PR-AUC baseline in that test period is about the late rate, 5.3%, so the model ranks risk better than chance. The hard threshold is conservative and should be tuned to the business action: monitoring, seller contact, ETA adjustment, shipping upgrade, or proactive customer messaging.
